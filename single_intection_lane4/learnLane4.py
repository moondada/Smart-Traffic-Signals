...
GAMMA = 0.8 # reward 的衰减因子，一般取 0.9 到 0.999 不等
...
traci.start([sumoBinary, "-c", "data/lane4.sumocfg",
...
while len(rpm) < MEMORY_WARMUP_SIZE:
    if traci.simulation.getMinExpectedNumber() <= 0:
        traci.load(["--start", "-c", "data/lane4.sumocfg",
                    "--tripinfo-output", "tripinfo.xml"])
    run_episode(agent, rpm)
...
while episode < max_episode:  # 训练max_episode个回合，test部分不计算入episode数量
    ...
    traci.trafficlight.setPhase("0", 0)
    # train part
    total_reward, sum_q_lens = run_episode(agent, rpm)
    episode += 1
    AVG_Q_len_perepisode.append(sum_q_lens / 702)


# 训练结束，保存模型
save_path = './amodel/lane4_model.ckpt'
